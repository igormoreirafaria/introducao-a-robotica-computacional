function robotica_trabalho_1
  ...
  while(1)
    ...
    for i=1:2
      for j=1:2
        if  (i~=j)
          if mean(dist_obj_rob) <= Robo(1).raio+Objeto(1).raio
            Robo(i)=Repulsao(Robo(i),Robo(j),0.1);
          end
        end
      end
    end
    ...
    for i=1:2
      for j=i:2
        if (i~=j)
          aux = (Robo(i).pos(1)-Robo(j).pos(1)).^2+(Robo(i).pos(2)-Robo(j).pos(2)).^2 
          if any(aux == a2)
            continue
          else
    ...
    for i=3:4
      for j=3:4
        if  (i~=j)
          if mean(dist_obj_rob) <= Robo(1).raio+Objeto(2).raio
            Robo(i)=Repulsao(Robo(i),Robo(j),0.1);
          end
        end
      end
    ...
    if (sqrt(b2(1)) < 50) && sqrt((Robo(i).pos(1)-Objeto(2).pos(1)).^2+ (Robo(i).pos(2)-Objeto(2).pos(2)).^2) < 40
      Objeto(2)=Atracao(Objeto(2),Alvo(2),0.5); 
    end

    if  sqrt((Objeto(1).pos(1)-Alvo(1).pos(1)).^2+ (Objeto(1).pos(2)-Alvo(1).pos(2)).^2) < 40 && sqrt((Objeto(2).pos(1)-Alvo(2).pos(1)).^2+ (Objeto(2).pos(2)-Alvo(2).pos(2)).^2) < 40
      break
    end 
    
    if mod(n,10)==0
      Exibir_Arena([600 600],Robo,Alvo,Obstaculo,Objeto);
    end
  end
end
